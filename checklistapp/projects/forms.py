...
#         self.fields["step_templates"].queryset = StepTemplate.objects.filter(
#             is_active=True
...
#         widget=forms.HiddenInput(), help_text="JSON array of step IDs in desired order"
#     )
...
class ProjectSetupFormV2(forms.Form):
    ...
    def save(self):
        ...
        current_max_order = (
            ProjectStep.objects.filter(project=self.instance).aggregate(
                models.Max("order")
            )["order__max"]
            or 0
        )
        ...
        project_step = ProjectStep.objects.create(
            ...
            icon=getattr(
                self.step_template, "icon", "?"
            ),  # Safely access icon if it exists
            order=current_max_order + 1,
        )

        # Create tasks from template
        # Assuming 'tasks' is the related_name from StepTemplate to TaskTemplate
        task_templates = self.step_template.tasks.filter(is_active=True).order_by(
            "order"
        )
        for j, task_template in enumerate(task_templates, start=1):
            ProjectTask.objects.create(
                ...
                info_url=getattr(
                    task_template, "info_url", ""
                ),  # Safely access info_url
                order=j,
            )
...
class ProjectSetupForm(forms.Form):
    ...
    def __init__(self, *args, instance=None, **kwargs):
        ...
        for template in self.available_templates:
            ...
            self.fields[f"{field_prefix}_quantity"] = forms.IntegerField(
                widget=forms.TextInput(
                    attrs={"min": 0, "max": "10", "type": "number", "value": 1}
                )
            )
    ...
    def save(self, commit=True):
        ...
        quantity_fields = sorted(
            [k for k in self.cleaned_data if k.endswith("_quantity")]
        )

        for quantity_field in quantity_fields:
            template = self.fields[
                quantity_field
            ].template  # Get the template instance we attached
            ...
            custom_names = [
                name.strip() for name in custom_names_text.split("\n") if name.strip()
            ]
...
class QuickAddStepForm(forms.Form):
    ...
    step_template_id = forms.IntegerField(widget=forms.HiddenInput())
...
class StepDuplicateForm(forms.Form):
    ...
    def save(self):
        ...
        return ProjectService.duplicate_step(
            project_step=self.step, custom_title=custom_title
        )
